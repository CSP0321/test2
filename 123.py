...
修改点说明：
确保日期格式正确。如果日期格式不正确，使用 st.error 提示用户并停止代码执行。
在读取数据并转换为字典后，将日期从时间戳转换为 datetime 格式以确保一致性。
在画图过程中使用 make_subplots 以便在同一个图中包含不同的Y轴（如成交量和价格）。
使用 st.expander 来折叠和展开不同的图表，改善用户界面的美观性和可操作性。
你可以运行这个代码并检查输出。如果有任何问题或错误，请告诉我，我可以进一步帮助你调试。
...
File "/mount/src/test2/123.py", line 34, in <module>
    df_original = df_original.drop('Unnamed: 0',axis=1)
                  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/home/adminuser/venv/lib/python3.11/site-packages/pandas/core/frame.py", line 5581, in drop
    return super().drop(
           ^^^^^^^^^^^^^
File "/home/adminuser/venv/lib/python3.11/site-packages/pandas/core/generic.py", line 4788, in drop
    obj = obj._drop_axis(labels, axis, level=level, errors=errors)
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/home/adminuser/venv/lib/python3.11/site-packages/pandas/core/generic.py", line 4830, in _drop_axis
    new_axis = axis.drop(labels, errors=errors)
               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/home/adminuser/venv/lib/python3.11/site-packages/pandas/core/indexes/base.py", line 7070, in drop
    raise KeyError(f"{labels[mask].tolist()} not found in axis")
ChatGPT
从错误信息来看，你试图删除 Unnamed: 0 列，但该列在你的数据中并不存在。为了避免这个问题，我们可以在删除列之前先检查该列是否存在。

另外，为了更加健壮地处理数据，我们可以在整个数据处理链条上添加更多的错误处理和日志信息。

以下是改进后的代码，增加了列存在性检查和详细的日志输出：
