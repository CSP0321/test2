...
import indicator, order_shioaji, KD_realTrade_Shioaji
...
end_date = st.text_input('選擇結束日期 (日期格式: 2024-04-30)', '2019-04-30')
...
time_unit = st.selectbox("選擇時間單位", ["小時", "日"], key="time_unit")
# 根據選擇的時間單位輸入 K 棒的時間長度
if time_unit == "小時":
    cycle_duration = st.number_input('輸入一根 K 棒的時間長度(單位:小時)', key="KBar_duration_hours")
    cycle_duration_minutes = int(cycle_duration) * 60
else:
    cycle_duration = st.number_input('輸入一根 K 棒的時間長度(單位:日)', key="KBar_duration_days")
    cycle_duration_minutes = int(cycle_duration) * 1440
...
with st.expander("K線圖, KDJ指標, 實單交易策略"):
    ...
    #### include KDJ indicators
    fig3.add_trace(go.Scatter(x=KBar_df['Time'], y=KBar_df['K'], mode='lines',line=dict(color='purple', width=2), name='K值'), secondary_y=False)
    fig3.add_trace(go.Scatter(x=KBar_df['Time'], y=KBar_df['D'], mode='lines',line=dict(color='green', width=2), name='D值'), secondary_y=False)
    fig3.add_trace(go.Scatter(x=KBar_df['Time'], y=KBar_df['J'], mode='lines',line=dict(color='orange', width=2), name='J值'), secondary_y=False)

    #### include trading signals
    fig3.add_trace(go.Scatter(x=KBar_df[KBar_df['Buy_Signal']]['Time'], y=KBar_df[KBar_df['Buy_Signal']]['Close'], mode='markers', marker=dict(color='blue', size=10, symbol='triangle-up'), name='買入信號'), secondary_y=True)
    fig3.add_trace(go.Scatter(x=KBar_df[KBar_df['Sell_Signal']]['Time'], y=KBar_df[KBar_df['Sell_Signal']]['Close'], mode='markers', marker=dict(color='red', size=10, symbol='triangle-down'), name='賣出信號'), secondary_y=True)
...
with st.expander("K線圖, 布林通道"):
    ...
    fig4.add_trace(go.Scatter(x=KBar_df['Time'], y=KBar_df['BB_upper'], mode='lines',line=dict(color='red', width=2), name='布林通道上軌'), secondary_y=True)
    fig4.add_trace(go.Scatter(x=KBar_df['Time'], y=KBar_df['BB_middle'], mode='lines',line=dict(color='blue', width=2), name='布林通道中軌'), secondary_y=True)
    fig4.add_trace(go.Scatter(x=KBar_df['Time'], y=KBar_df['BB_lower'], mode='lines',line=dict(color='green', width=2), name='布林通道下軌'), secondary_y=True)
